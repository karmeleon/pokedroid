...
import com.games.pokedroid.game.GUIBattle.Messenger;
...
public class Attack
{
	 ...
	 */
	public void attackPhase(Context c, Move userAttack, Move opponentAttack, Pokemon user, Pokemon[] yourteam,  Pokemon opponent, Pokemon[] opponentteam, Battlefield b, Messenger h){
		...
		if(userAttack.isSwitch&&opponentAttack.isSwitch){
			if(user.effectiveSpeed>opponent.effectiveSpeed){
	    		...
	    		user=GUIBattle.team1[GUIBattle.activeAlly];
	            opponent=GUIBattle.team2[GUIBattle.activeEnemy];
	            yourteam=GUIBattle.team1;
	            opponentteam=GUIBattle.team2;
	...
	private void BattlePhase(Move move, Pokemon user, Pokemon target, Battlefield bf, Pokemon[] userTeam, Pokemon[] targetTeam)
	{
						...
						if(!attackHits||(Pokemon.getEffectiveness(move.type, target.type1)*Pokemon.getEffectiveness(move.type, target.type2)==0&&move.moveTarget==1)){
							...
							if(move.DamageType==move.NONDAMAGING){
								switch(move.effect){
									...
									case 10:{
										if(((whoGoesFirst==1&&isUserAttacking)||(whoGoesFirst==0&&!isUserAttacking))&&lastUsedMove.moveFlags.contains(7)){
											...
											moveFails(move);
												...
												if(isUserAttacking){
													GUIBattle.team1=userTeam;
													GUIBattle.team2=targetTeam;
													GUIBattle.team1[GUIBattle.activeAlly]=user;
													GUIBattle.team2[GUIBattle.activeEnemy]=target;
												}
												else{
													GUIBattle.team1=targetTeam;
													GUIBattle.team2=userTeam;
													GUIBattle.team1[GUIBattle.activeEnemy]=target;
													GUIBattle.team2[GUIBattle.activeAlly]=user;
												}
									...
									case 83:{
										if(isUserAttacking&&opponentLastUsedMove==null||!isUserAttacking&&userLastUsedMove==null)
											moveFails(move);
										else if(whoGoesFirst==0&&(opponentLastUsedMove.justFailed||opponentLastUsedMove.name.equals("Chatter")||opponentLastUsedMove.name.equals("Metronome")||opponentLastUsedMove.name.equals("Mimic")||opponentLastUsedMove.name.equals("Sketch")||opponentLastUsedMove.name.equals("Struggle")))
											moveFails(move);
										else if(whoGoesFirst==1&&(userLastUsedMove.justFailed||userLastUsedMove.name.equals("Chatter")||userLastUsedMove.name.equals("Metronome")||userLastUsedMove.name.equals("Mimic")||userLastUsedMove.name.equals("Sketch")||userLastUsedMove.name.equals("Struggle")))
											moveFails(move);
										else{
											user.MimicPP=move.CurrentPP-1;
											
											if(!isUserAttacking){
												if(user.attack1.name.equals("Mimic")){
													user.attack1=userLastUsedMove;
													user.attack1.CurrentPP=6;
													user.isMimicing=1;
												}
												else if(user.attack2.name.equals("Mimic")){
													user.attack2=userLastUsedMove;
													user.attack2.CurrentPP=6;
													user.isMimicing=2;
												}
												else if(user.attack3.name.equals("Mimic")){
													user.attack3=userLastUsedMove;
													user.attack3.CurrentPP=6;
													user.isMimicing=1;
												}
												else if(user.attack4.name.equals("Mimic")){
													user.attack4=userLastUsedMove;
													user.attack4.CurrentPP=6;
													user.isMimicing=1;
												}
												
												message(user.name + " copied " + userLastUsedMove.name + "!");
											}
											if(isUserAttacking){
												if(user.attack1.name.equals("Mimic")){
													user.attack1=opponentLastUsedMove;
													user.attack1.CurrentPP=6;
													user.isMimicing=1;
												}
												else if(user.attack2.name.equals("Mimic")){
													user.attack2=opponentLastUsedMove;
													user.attack2.CurrentPP=6;
													user.isMimicing=2;
												}
												else if(user.attack3.name.equals("Mimic")){
													user.attack3=opponentLastUsedMove;
													user.attack3.CurrentPP=6;
													user.isMimicing=3;
												}
												else if(user.attack4.name.equals("Mimic")){
													user.attack4=opponentLastUsedMove;
													user.attack4.CurrentPP=6;
													user.isMimicing=4;
												}
												
												message(user.name + " copied " + opponentLastUsedMove.name + "!");
											}
										}	
										break;
									}
									case 85:{
										if(target.isLeechSeeded==false&&target.type1!=Pokemon.GRASS&&target.type2!=Pokemon.GRASS){
										target.isLeechSeeded=true;
										...
										if(user.isAsleep){
											...
											while(!work){
												if(sleepTalkCheck<.25&&move.compareTo(user.attack1)!=0){
													BattlePhase(user.attack1, user, target, bf, userTeam, targetTeam );
													work=true;
												}
												else if(sleepTalkCheck<.5&&move.compareTo(user.attack2)!=0){
													BattlePhase(user.attack2, user, target, bf, userTeam, targetTeam );
													work=true;
												}
												else if(sleepTalkCheck<.75&&move.compareTo(user.attack3)!=0){
													BattlePhase(user.attack3, user, target, bf, userTeam, targetTeam );
													work=true;
												}
												else{
													BattlePhase(user.attack4, user, target, bf, userTeam, targetTeam );
													work=true;
												}
											}
											attackIsSleepTalk=false;
		...
		if(user.attack1.compareTo(move)==0)
			user.attack1.CurrentPP--;
		if(user.attack2.compareTo(move)==0)
			user.attack2.CurrentPP--;
		if(user.attack3.compareTo(move)==0)
			user.attack3.CurrentPP--;
		if(user.attack4.compareTo(move)==0)
			user.attack4.CurrentPP--;
	...
	private void endPhase(Pokemon user, Pokemon opponent, Battlefield b, Pokemon[] yourteam, Pokemon[] opponentteam){
		...
		if(!user.hasFainted){
			...
			if(user.isLeechSeeded){
				user.damage(user.maxHP/8);
				if(user.chosenAbility!=Pokemon.LIQUID_OOZE)
					opponent.heal(user.maxHP/8);
				else
					opponent.damage(user.maxHP/8);
		...
		if(!opponent.hasFainted){
			...
			if(opponent.isLeechSeeded){
				opponent.damage(opponent.maxHP/8);
				if(opponent.chosenAbility!=Pokemon.LIQUID_OOZE)
					user.heal(opponent.maxHP/8);
				else
					user.damage(opponent.maxHP/8);
		...
		if(user.hasFainted){
			...
			int switchTo=GUIBattle.onPlayerFaint();
			switchPokemon(user,yourteam,switchTo,b, 1);
			}
		}
		
		if(opponent.hasFainted){
			int AISwitch=TrainerAI.onAIFaint(user, opponent, b, opponentteam);
			if(!(AISwitch==-2))	
		...
		//GUIBattle.team1[GUIBattle.activeAlly]=user;
		//GUIBattle.team2[GUIBattle.activeEnemy]=opponent;
		//GUIBattle.team1=yourteam;
		//GUIBattle.team2=opponentteam;
	...
	private void switchPokemon(Pokemon switchee, Pokemon[] switchTeam, int switchTo, Battlefield bf, int side){
		...
		GUIBattle.switchPokemon(side, switchTo, h);
		...
		if(side==1){
			GUIBattle.team1=switchTeam;
			GUIBattle.team1[0]=switchee;
		}
		else{
			GUIBattle.team2=switchTeam;
			GUIBattle.team2[0]=switchee;
		}
	...
	private void onSwitchIn(Pokemon switchee, Battlefield bf, int side){
		...
		if(side==1){
			GUIBattle.team1[GUIBattle.activeAlly]=switchee;
		}
		else{
			GUIBattle.team2[GUIBattle.activeEnemy]=switchee;
		}
	 ...
	 */
 	...
 	public void moveFails(Move move){
 		message("But it failed!");
 		move.justFailed=true;
 	}
	...
	public static void message(String message)
	{
		...
		m.setData(data);
		h.sendMessageDelayinated(m, MESSAGE_DELAY);
	}
